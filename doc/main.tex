...
\begin{itemize}
    \item static-$\lambda$: $\lambda$ is fixed for the whole run of \opll. Tuning budget for this setting is $n \times 100$.  
    \item dynamic-$\lambda$: we search for a policy $p$ that maps each $f(x)$ to a specific $\lambda$. In other words, the number of parameters to be tuned is equal to the problem size $n$. The tuning budget for this is 1 million or $n$ multipied by $10000$, whichever is smaller.
\end{itemize}
