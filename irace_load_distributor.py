...
def run_full(i, dynamic_seed, dynamic_bin_seed, static_seed, grapher_seed, job_type: JobType):
...
def main(job_type: JobType):
  runs = [Thread(target=run_full, args=(i, rng.integers(1<<15, (1<<16)-1), rng.integers(1<<15, (1<<16)-1), rng.integers(1<<15, (1<<16)-1), rng.integers(1<<15, (1<<16)-1), job_type)) for i in range(N)]
  for thread in runs:
    thread.start()
  for thread in runs:
    thread.join()
