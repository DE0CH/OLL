...
import oll.xover.LegacyCollectiveCrossoverComputation
import util.Viridis

object LandscapeAnalysis {
  private def varyingLastTwoLambdas(cmd: CommandLineArgs): Unit = {
    val n = 500
    ...
    Using.resource(Files.newOutputStream(Paths.get("landscape-analysis-last-two-lambdas.png")))(stream => ImageIO.write(image, "png", stream))
  }

  private def varyingLastLambda(cmd: CommandLineArgs): Unit = {
    val n = 500
    val bins = RunGivenLambdas.defaultBins(n)
    val lambdas = Array(1.0, 1.0, 1.0, 1.0, 6.5, 8.5, 11.5, 16.5, 24.5)
    ...
    Using.resource(new PrintWriter("landscape-analysis-last-lambda.csv")) { out =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    varyingLastLambda(cmd)
    varyingLastTwoLambdas(cmd)
  }
}
